    ...
    std::vector<CLSR::Vertex> &vertices)
{
    ...
    std::vector<CLSR::Vertex> disk_vertices;
    ...
    for (auto& cluster : clusters) {
        ...
        for (uint32_t triangle_i : cluster.triangles) {
            ...
            CLSR::Vertex& v0 = vertices[triangle_verts[0]];
            CLSR::Vertex& v1 = vertices[triangle_verts[1]];
            CLSR::Vertex& v2 = vertices[triangle_verts[2]];
    ...
    clsr_file.write(reinterpret_cast<const char *>(disk_vertices.data()), sizeof(CLSR::Vertex) * disk_vertices.size());
...
void read_clsr(std::string file_path, RuntimeDAG* to, bool debug) {
    ...
    while (std::filesystem::exists(file_path + "_" + std::to_string(LOD_level) + ".clsr")) {
        ...
        to->vertices.push_back(std::vector<CLSR::Vertex>(vertices_count));
        ...
        if (!cluster_file.read(reinterpret_cast< char * >(to->vertices[LOD_level].data()), sizeof(CLSR::Vertex) * vertices_count)) {
